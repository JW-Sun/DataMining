...
labels = ['property1', 'property2','property3','property4']
...
def calcShannonEnt(dataSet):
    ...
    for key in labelCounts:
        ...
        shannonEnt -= prob * log(prob, 2)  # 计算乘积,以2为底数
    return shannonEnt
...
def createTree(dataSet,labels):     #创建树的函数代码
    ...
    pure = data.most_common(1)[0][1] / len(classList)  # 计算纯度
    if pure>0.95:#如果纯度至少为95％，则停止
        return "type:" + data.most_common(1)[0][0] + " pure:" + str(pure) + " size:" + str(len(dataSet))
    if len(dataSet) <= 5:
        return "type:" + data.most_common(1)[0][0] + " purity:" + str(pure) + " size:" + str(
            len(dataSet))  # 第二个递归结束条件：节点数量小于5
...
tree=createTree(data2a,labels)
out = open('DecisonTree.txt', 'w')
print(tree, file=out)
out.close()
